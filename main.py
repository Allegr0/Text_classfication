...
def nbClassifier(trainLabel,testLabel):
...
def logisticReg(trainLabel,testLabel):
...
if __name__ == '__main__':
    ...
    for re in result:
        if 'fenci' in re:
            trainCorpus.append(re['fenci2'].strip())
            classLabel.append(re['label'])
    trainData, testData, trainLabel, testLabel = train_test_split(trainCorpus, classLabel, test_size = 0.5)
    ...
    f.close()
    print('Size of fea_train:' + str(fea_train.shape))
    print('Size of fea_test:' + str(fea_test.shape))
    print(fea_train.nnz)
    print(fea_test.nnz)
    end = time.time()
